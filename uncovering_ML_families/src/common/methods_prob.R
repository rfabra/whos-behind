...
runmethods <-function (mymethod, train, test) 
{
 ...
 # print(paste("Dataset: ",datasets[selected],", Method: ", mymethod,", fold: ",ik))
  ...
  if (mymethod=="c5.0") {
    ...
    ERROR <-  tryCatch(model <- C5.0(Class ~., data = train), error = function(e) {print(e);return(TRUE)})
    
    if (is.logical(ERROR)){
      preds <- rep(FALSE, nrow(test))
    }else{
      preds$crisp <- tryCatch(predict(model, newdata = test, type="class"), error = function(e) {return(rep(FALSE, nrow(test)))})
      preds$probs <- tryCatch(predict(model, newdata = test, type="prob"), error = function(e) {return(rep(FALSE, nrow(test)))})
    }
  ...
  if (mymethod=="c5.0_winnow"){
    ...
    if (is.logical(ERROR)){
      ...
      preds<- tryCatch(predict(model, newdata = test, type="prob"), error = function(e) {return(rep(FALSE, nrow(test)))})
    }
    tryCatch(detach("package:C50", unload=TRUE), error = function(e){print(e)})
  }
  ...
  if (mymethod == "ctree_c0.01"){
    ...
    if (is.logical(ERROR)){
      ...
      preds<- tryCatch(predict(model, newdata = test, type="prob"), error = function(e) {print(e); return(rep(FALSE, nrow(test)))})
    }
  ...
  if (mymethod == "ctree_c0.05"){
    ...
    if (is.logical(ERROR)){
      ...
      preds<- tryCatch(predict(model, newdata = test, type="prob"), error = function(e) {print("ExcepTION: "); print(e); return(rep(FALSE, nrow(test)))})
  ...
  if (mymethod == "NB") {
    ...
    if (is.logical(ERROR)){
      ...
      #preds<- tryCatch(predict(model, test[,1:ncol(test)-1]), error = function(e) {return(rep(FALSE, nrow(test)))})
      preds$crisp<- tryCatch(predict(model, test, type="class"), error = function(e) {return(rep(FALSE, nrow(test)))})
      preds$probs<- tryCatch(predict(model, test, type="prob"), error = function(e) {return(rep(FALSE, nrow(test)))})
      #print(preds)
    }
  ...
  if (mymethod == "svmRadialCost_C2_5") {
    load_install("kernlab")
    ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "svmRadialCost", tuneGrid=data.frame(C=2^5), trControl = trainControl(method="none", classProbs=TRUE)), error = function(e) {return(TRUE)})
    if (is.logical(ERROR)){
      preds <- rep(FALSE, nrow(test))
    }else{
      preds$crisp <- tryCatch(predict(model, newdata = test, type="raw"), error = function(e) {return(rep(FALSE, nrow(test)))})
      preds$probs <- tryCatch(predict(model, newdata = test, type="prob"), error = function(e) {return(rep(FALSE, nrow(test)))})
    }
    tryCatch(detach("package:kernlab", unload=TRUE), error = function(e){print(e)})
  }
  ...
  if (mymethod == "knn_k5") { 
    ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "knn",tuneGrid=data.frame(k = 5),  trControl = trainControl(method="none", classProbs = TRUE)), error = function(e) {return(TRUE)})
  ...
  if (mymethod == "simpls_ncomp4") { 
    load_install("pls")
    ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "simpls",tuneGrid=data.frame(ncomp = 4),  trControl = trainControl(method="none", classProbs = TRUE)), error = function(e) {return(TRUE)})
  ...
  if (mymethod == "gcvEarth_d3") { #135
    load_install("earth")
    ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "gcvEarth",tuneGrid=data.frame(degree = 3),  trControl = trainControl(method="none", classProbs = TRUE)), error = function(e) {return(TRUE)})
  ...
  if (mymethod == "glmnet") {
    load_install("glmnet")
    ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "glmnet", trControl = trainControl(classProbs=TRUE)), error = function(e) {print(e);return(TRUE)})

    if (is.logical(ERROR)){
      preds <- rep(FALSE, nrow(test))
    }else{
      preds$crisp <- tryCatch(predict(model, newdata = test, type="raw"), error = function(e) {print(e); return(rep(FALSE, nrow(test)))})
      preds$probs <- tryCatch(predict(model, newdata = test, type="prob"), error = function(e) {print(e); return(rep(FALSE, nrow(test)))})
    }
  ...
  if (mymethod == "mlpML") {
    ...
    ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc =  c("center", "scale"),method = "mlpML", trControl = trainControl(method="none", classProbs=TRUE)), error = function(e) {return(TRUE)})
...
# Method: multinom
	if (mymethod == "multinom") {
            load_install("nnet")
		ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "multinom", trControl = trainControl(method="none", classProbs=TRUE)), error = function(e) {print(e); return(TRUE)})
...
# Method: rda
	if (mymethod == "rda") {
            load_install("klaR")
		ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "rda"), error = function(e) {print(e); return(TRUE)})
...
# Method: wsrf
	if (mymethod == "wsrf") {
            load_install("wsrf")
		ERROR <-  tryCatch(model <- caret::train(Class ~ ., data = train, preProc = c("center", "scale"), method = "wsrf", trControl = trainControl(method="none", classProbs=TRUE)), trace=FALSE, error = function(e) {print(e); return(TRUE)})
...
# Method: glmboost
  ...
  preds$probs <- t(apply(preds$probs, 1, function(x){x/sum(x)}))
  return(preds)
}
