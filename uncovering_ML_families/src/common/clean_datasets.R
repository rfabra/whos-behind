...
library(entropy)
library(plyr)
source("1_family_dendrograms/save_load/save_load.R")
...
for (ds in datasets) {
  ...
  if (csv_data[["selected?"]][[ids]] == "YES") {
    save_dataset(current_data, gsub(".arff", ".csv", sprintf("datasets/clean_test/%s", ds)))
...
write.table(x=csv_data, file="1_family_dendrograms/files/analytics/datasets_clean2.csv", row.names = FALSE, col.names = TRUE, sep=",")
